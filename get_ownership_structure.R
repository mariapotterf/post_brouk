...
subplots_sf    <- st_as_sf(subplots_3035)

crs(owners_sf) == crs(subplots_sf)

ggplot() +
  geom_sf(data = aoi_sf, fill ="white", color = "black") +
  geom_sf(data = owners_sf, aes(fill = owner), color = NA) +
  geom_sf(data = subplots_sf, color = "red") +
